...
namespace Core.Utility.Extensions
{
    ...
    {
        public static string ToPyCode(this string str)
        {
            return GetChineseSpellCode(str);
        }

        private static string GetChineseSpellCode(string unicodeString)
        {
            var i = 0;
            ushort key = 0;
            var strResult = string.Empty;            //创建两个不同的encoding对象     
        ...
        /// <returns></returns>  
        private static string GetFirstPYLetter(string transName)
        {
            ...
            var return_py = "";
            foreach (var t in transName)
            {
                var array = Encoding.Unicode.GetBytes(t.ToString());
                if (array[0] < 176)  //.非汉字  
                {
                    return_py += t;
                }
                else if (array[0] >= 176 && array[0] <= 215)  //一级汉字  
                {
                    if (string.Compare(t.ToString(), "匝", StringComparison.Ordinal) >= 0)
                        ...
                        return_py += "x";
                    else if (string.Compare(t.ToString(), "挖", StringComparison.Ordinal) >= 0)
                        return_py += "w";
                ...
                {
                    return_py += ls_second_eng.Substring(ls_second_ch.IndexOf(t.ToString(), 0, StringComparison.Ordinal), 1);
                }
            }
        ...
        private static string GetAllPYLetters(string transName)
        {
            // 匹配中文字符  
            var regex = new Regex("^[\u4e00-\u9fa5]{1}");

            var pyString = "";
            var noWChar = transName.ToCharArray();
