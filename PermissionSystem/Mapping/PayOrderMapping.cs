// =================================================================== 
// 项目说明
//====================================================================
// YXL @ CopyRight 2006-2010
// 文件： PayOrderEntity.cs
// 项目名称：Asp.Net Core 2.0 mvc 开源权限系统Demo 
// 创建时间：2017-10-25
// 负责人：杨小乐
// ===================================================================
using Microsoft.EntityFrameworkCore.Metadata.Builders;
using PermissionSystem.Models;
namespace PermissionSystem.Mapping
{
	/// <summary>
	///PayOrder数据实体
	/// </summary>
	internal class PayOrderMapping
	{  
        
       public static void Mapping(EntityTypeBuilder<PayOrder> table)
       {
            table.HasKey(a=>a.ID); 
            table.Property(a=>a.ID).IsRequired();
            table.Property(a=>a.ClientID).IsRequired();
            table.Property(a=>a.OrderNumber).IsRequired();
            table.Property(a=>a.Amount).IsRequired();
            table.Property(a=>a.Remark);
            table.Property(a=>a.CreateDate).IsRequired();
            table.Property(a=>a.RecordDate).IsRequired();
       
            table.HasMany(a => a.PaySerial_PayOrderIDList).WithOne(a => a.PayOrderID_Model);       
        
            table.HasOne(a => a.ClientID_Model).WithMany(a => a.PayOrder_ClientIDList);
        
        
      
       }
	}
	
}